...
public class SmscSenderImpl implements SmsSender {

    private String SMSC_LOGIN = System.getProperty("volley.sms.login");
    private String SMSC_PASSWORD = System.getProperty("volley.sms.pass");
    ...
    public String[] sendSms(String phones, String message, int translit, String time, String id, int format, String sender, String query) {
        ...
        try {
            m = _smsc_send_cmd("send", "cost=3&phones=" + URLEncoder.encode(phones, SMSC_CHARSET)
                ...
                + (Objects.equals(sender, "") ? "" : "&sender=" + URLEncoder.encode(sender, SMSC_CHARSET))
                + (Objects.equals(time, "") ? "" : "&time=" + URLEncoder.encode(time, SMSC_CHARSET))
                + (Objects.equals(query, "") ? "" : "&" + query));
    ...
    public String[] getStatus(int id, String phone, int all) {
        ...
        try {
            ...
            if (SMSC_DEBUG) {
                if (!Objects.equals(m[1], "") && Integer.parseInt(m[1]) >= 0) {
                    System.out.println("Статус SMS = " + m[0]);
                } else
                    System.out.println("Ошибка №" + Math.abs(Integer.parseInt(m[1])));
            }

            if (all == 1 && m.length > 9 && (m.length < 14 || !Objects.equals(m[14], "HLR"))) {
                String tmp = String.join(",", m);
                m = tmp.split(",", 9);
            }
        } catch (UnsupportedEncodingException ignored) {
