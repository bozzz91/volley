(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope, $translate) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('volleyApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-volleyApp-error');
                var entityKey = httpResponse.headers('X-volleyApp-params');
                if (errorHeader) {
                    ...
                    for (i = 0; i < httpResponse.data.fieldErrors.length; i++) {
                        ...
                        var fieldName = $translate.instant('volleyApp.' + fieldError.objectName + '.' + convertedField);
